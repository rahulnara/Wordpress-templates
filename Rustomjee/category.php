...
?>
    ...
    <section class="img_content_slider_main clearfix">
        <div class="img_content_slider clearfix">
            <?php 
				while ( $query->have_posts() ) : $query->the_post(); $ct_cnt++;
                    $d_exclude[] = get_the_ID();
				    if($ct_cnt > 3){break;}
            ?>
    ...
    <section class="blog_listing_main clearfix">
        ...
        <div class="blog_items_list clearfix">
            <?php 
                $cat_args['post__not_in'] = $d_exclude;
                rst_loadmore_ajax_handler($cat_args); ?>
            <?php
                global $wp_query;
                if (  $wp_query->max_num_pages > 1 )
                    echo '<div class="load_more_blog_wrap">
                <a href="javascript:void(0)" class="btn load_more_blog_cta">Load more</a>
            </div>'; ?>
        </div>
    </section>
<?php else: ?>
    <section class="no_blog_section clearfix">
        <div class="nbs_data">
            <h1>Nothing Found</h1>
